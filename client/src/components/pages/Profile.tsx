...
import { RecipeModel, StockedItem } from "../../types";
		...
		const load = async () => {
			try {
				...
				if (authUser) {
					...
					const profileResp = await apiClient.get<{ user?: User }>(
						"/profile"
					);
					if (!mounted) return;
				...
				try {
					const r = await apiClient.get<RecipeModel[]>(
						"/api/recipes"
					);
					if (mounted && Array.isArray(r.data)) setRecipes(r.data);
				} catch (err) {
					console.debug(
						"Profile: unable to fetch recipes index:",
						err
					);
			...
			<div className="profile-card">
				<div className="profile-header">
					...
					<div className="profile-meta">
						...
						{saveError && (
							<p style={{ color: "#ffb4b4", marginTop: 6 }}>
								{saveError}
							</p>
						)}
					</div>
								...
								{(() => {
									...
									const recipesByMealType =
										filteredRecipes.reduce(
											...
											{} as {
												[key: string]: RecipeModel[];
											}
										);

									// Sort recipes within each meal type
									Object.keys(recipesByMealType).forEach(
										(mealType) => {
											recipesByMealType[mealType].sort(
												(a, b) =>
												...
												{recipesByMealType[
													...
													<li key={recipe.id}>
														<button
															...
															onClick={() => {
																// Set the recipe to display in the context (fast path)
																if (
																	setRecipeToDisplay
																)
																	setRecipeToDisplay(
																...
																// Navigate to canonical recipe detail route and pass the recipe in state
																navigate(
																	`/recipes/${recipe.id}`,
																	{
																		state: {
